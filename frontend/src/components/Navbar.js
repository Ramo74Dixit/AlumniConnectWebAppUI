  ...
  return (
    <div className='w-full h-[12vh] border shadow-lg flex items-center justify-between px-4 md:px-10 z-50'>
      <div className="flex items-center">
        <img src="/logo.png" className='w-[20vh] h-[10vh] md:w-[30vh] md:h-[12vh]' alt="Logo" />
        <div className='font-bold text-2xl md:text-xl ml-0 md:ml-0'>AlumniConnect</div>
      </div>
      ...
      <div className={`md:hidden absolute top-[12vh] left-0 w-full bg-white shadow-lg ${isOpen ? 'block' : 'hidden'}`}>
        <div className="flex flex-col space-y-4 p-4 text-lg font-normal text-[#8A8A8A]">
          ...
          <Link to="/alumnidirectory" onClick={toggleMenu}>Alumni Directory</Link>
          <Link to="/jobportal" onClick={toggleMenu}>Job Portal</Link>
          <Link to="/events" onClick={toggleMenu}>Events</Link>
          <Link to="/donation" onClick={toggleMenu}>Donation</Link>
